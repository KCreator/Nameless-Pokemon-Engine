...
void DialogFrame::Render( int StartX, int StartY, int EndX, int EndY )
{
	...
	SDL_RenderCopy( gRenderer, m_texture, &GetRect(16, 0, 8, 8 ), &GetRect( EndX - 8, StartY, 8, 8 ) );
	...
	SDL_RenderCopy( gRenderer, m_texture, &GetRect(16, 8, 8, 8 ), &GetRect( EndX - 8, StartY+8, 8, EndY - 8 ) );

	//Bottom
	SDL_RenderCopy( gRenderer, m_texture, &GetRect(0, 16, 8, 8 ), &GetRect( StartX, EndY-8, 8, 8 ) );
	SDL_RenderCopy( gRenderer, m_texture, &GetRect(8, 16, 8, 8 ), &GetRect( StartX + 8, EndY-8, EndX-8, 8 ) );
	SDL_RenderCopy( gRenderer, m_texture, &GetRect(16, 16, 8, 8 ), &GetRect( EndX - 8, EndY-8, 8, 8 ) );
}